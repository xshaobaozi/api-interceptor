...
export default defineStore(ID, {
  ...
  actions: {
      ...
      getStore(keys, (result) => {
        keys.forEach((key) => {
          ...
          if (getSource !== undefined && getSource !== null) {
            if (key === 'apis') {
              this[key] = Array.from(getSource).map((item) => ({
                ...item,
                schema: parse(item.schema),
              }));
              return;
            }
            this[key] = getSource;
    ...
    editItem(formValue) {
      ...
      const { uri, methods } = formValue;
      try {
        target['schema']['paths'][uri][methods][wapperKey] = formValue;
        this.saveLocal();
      } catch (err) {
